...
2. Every error has a name. Don't say "handle errors." Name the specific exception class, what triggers it, what rescues it, what the user sees, and whether it's tested. Generic catch-all error handling is a code smell — call it out.
...
* ASCII diagrams in code comments for complex designs — Models (state transitions), Services (pipelines), Routes (request flow), and Tests (non-obvious setup).
...
grep -r "TODO\|FIXME\|HACK\|XXX" --include="*.ts" -l
```
...
```
  ...
  -------------------------|-----------------------------|-----------------
  MatchingEngine#generate  | API timeout                 | Anthropic timeout
                           | API returns 429             | RateLimitError
                           | API returns malformed JSON  | JSON.parse error
                           | DB query fails              | SQLite error
                           | No executives found         | Empty result set
  -------------------------|-----------------------------|-----------------

  ERROR TYPE                   | HANDLED?  | HANDLER ACTION         | USER SEES
  -----------------------------|-----------|------------------------|------------------
  Anthropic timeout            | Y         | Retry 2x, then throw   | "Service temporarily unavailable"
  RateLimitError               | Y         | Backoff + retry         | Nothing (transparent)
  JSON.parse error             | N ← GAP   | —                      | 500 error ← BAD
  SQLite error                 | N ← GAP   | —                      | 500 error ← BAD
  Empty result set             | Y         | Return []               | "No matches found"
```
Rules for this section:
* Generic `catch (error)` is ALWAYS a smell. Name the specific error types.
...
* For each GAP (unhandled error that should be handled): specify the handler action and what the user should see.
...
Evaluate:
* N+1 queries. For every new database query in a loop: is it batched?
* Memory usage. For every new data structure: what's the maximum size in production?
* Database indexes. For every new query: is there an index?
* Caching opportunities. For every expensive computation or external call: should it be cached?
* Background job sizing. For every new async task: worst-case payload, runtime, retry behavior?
...
* Ecosystem fit. Aligns with Bun/TypeScript ecosystem direction?
...
Complete table of every method that can fail, every error type, handled status, handler action, user impact.
