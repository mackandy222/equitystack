...
3. Run `git fetch origin main --quiet 2>/dev/null || git fetch origin master --quiet` and `git diff origin/main --stat 2>/dev/null || git diff origin/master --stat` to check if there's a diff. If no diff, output the same message and stop.
...
Run `git diff origin/main 2>/dev/null || git diff origin/master` to get the full diff. This includes both committed and uncommitted changes against the latest main.
...
2. **Pass 2 (INFORMATIONAL):** Conditional Side Effects, Magic Numbers & String Coupling, Dead Code & Consistency, LLM Prompt Issues, Test Gaps, Type Coercion at Boundaries
